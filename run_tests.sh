...
let total=0;
let correct=0;
export CLASSPATH=./xxl-app/xxl-app.jar:./xxl-core/xxl-core.jar:./po-uilib/po-uilib.jar:.
make -s tests;
for x in "$tests/"*.in; do
    ...
    if [ -e "${import_file}" ]; then
        java -Dimport="${import_file}" -Din="${in_file}" -Dout="${actual_output_file}" xxl.app.App;
    else
        java -Din="${in_file}" -Dout="${actual_output_file}" xxl.app.App;
    fi
